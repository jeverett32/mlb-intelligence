...
All times in `data/mlb_2026.csv` are **UTC**. The system runs 5 minutes before each game's UTC start time. Example: ATL plays ATH at 14:<REDACTED_PORT> UTC — the fetch runs at 13:<REDACTED_PORT> UTC, refreshing today's data and backfilling any unscored games from the previous day.
