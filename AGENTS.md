...
dashboard/app.py     # FastAPI dashboard (deployed on app LXC, port 8080)
...
python3 homelab.py app "curl -s http://localhost:8080/health"
...
#   run: python3 homelab.py db f"PGPASSWORD={pw} psql -U {user} -d {db} -h 127.0.0.1 -c '\\dt'"
```
